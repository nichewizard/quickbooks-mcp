...
const toolHandler = async ({ params }: any) => {
  ...
  if (response.isError) {
    return { content: [{ type: "text" as const, text: `Error searching sales receipts: ${response.error}` }] };
  }
  return {
    content: [
      { type: "text" as const, text: `Found ${response.result.length} sales receipt(s):` },
